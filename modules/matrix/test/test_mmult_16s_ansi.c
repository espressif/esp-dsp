...
static const char *TAG = "dslm_mult_16s_ansi";

// Test dsls_dotprod_16s_ansi function
TEST_CASE("dslm_mult_16s_ansi functionality", "[dslm]")
{
    int m = 4;
    int n = 3;
    int k = 4;
    ...
    for (int i=0 ; i< m ; i++)
    {
        for (int j=0 ; j< k ; j++)
        {
            ESP_LOGD(TAG, "[%i][%i] calc=%i, expected =%i\n",i,j, C[i][j], C_compare[i][j]);
        }
    }
...
TEST_CASE("dslm_mult_16s_ansi benchmark", "[dslm]")
{
    int m = 4;
    int n = 4;
    int k = 4;
