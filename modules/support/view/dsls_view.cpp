...
void dsls_view(float* data, int32_t len, int width, int height, float min, float max, char view_char)
{
    ...
    {
        int x_pos = i*x_step;
        if (data[i] < view_data_min[x_pos]) view_data_min[x_pos] = data[i];
        if (data[i] > view_data_max[x_pos]) view_data_max[x_pos] = data[i];

        if (view_data_min[x_pos] < min) view_data_min[x_pos] = min;
        if (view_data_max[x_pos] > max) view_data_max[x_pos] = max;
